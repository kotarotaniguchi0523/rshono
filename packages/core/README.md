...
- **Soft navigation needs the [Navigation API](https://developer.mozilla.org/en-US/docs/Web/API/Navigation_API)**
  ...
  which a server-rendered app answers correctly; only the soft part is gone. A push's reset — and its fragment
  jump — is the runtime's own, because WebKit and Chromium skip the browser's own reset for an intercepted
  push ([bugs.webkit.org 304593](https://bugs.webkit.org/show_bug.cgi?id=304593)); a traversal's restoration
  and the post-navigation focus reset are the browser's.
